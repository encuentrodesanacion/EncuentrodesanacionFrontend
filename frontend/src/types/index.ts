...
export interface RawDisponibilidadDBItem {
  ...
  id?: number; // Puede que venga o no, lo hacemos opcional
  terapeuta_id?: number; // Puede que venga o no, lo hacemos opcional
  created_at?: string;
  updated_at?: string;
}
...
export interface DisponibilidadTerapeuta {
  ...
  disponibilidadPorFecha: {
    [fecha: string]: string[]; // Ejemplo: { "2025-07-01": ["16:00", "18:00"], ... }
  };
}
